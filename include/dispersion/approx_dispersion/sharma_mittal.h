...
template <typename M>
struct DispersionTraits<ApproxDispersion<ApproxSharmaMittal<M> > >
{
  typedef M Model;
...
template <typename M>
class ApproxSharmaMittal : public ApproxDispersion<ApproxSharmaMittal<M> >
{
 public:
  typedef M Model;
  typedef typename Model::T T;
 ...
 public:
  ApproxSharmaMittal(const T& dimScaleMax, const T& offset = T(5.0),
                     const T& lambda = T(0.0), const int ksize = 3,
                     const T& alpha = T(2.0), const T& beta = T(0.5))
      : ApproxDispersion<ApproxSharmaMittal<Model> >(dimScaleMax, offset,
                                                     lambda, ksize),
        alpha_(alpha),
        beta_(beta),
