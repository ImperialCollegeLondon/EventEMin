...
main(void)
{
  ...
  // model
  typedef model::Homography<T> Model;
  ...
  const std::string fdir("../dataset/poster_6dof");
  ...
  vec<T, 3> w(optimiser.vars().template head<3>());
  vec<T, 3> v(optimiser.vars().template segment<3>(3));
  vec<T, 3> n;
  n<<std::cos(optimiser.vars()(6))*std::sin(optimiser.vars()(7)), std::sin(optimiser.vars()(6))*std::sin(optimiser.vars()(7)), std::cos(optimiser.vars()(7));
  std::cout<<", w: "<<w.transpose()<<", v: "<<v.transpose()<<", n: "<<n.transpose()<<'\n';
