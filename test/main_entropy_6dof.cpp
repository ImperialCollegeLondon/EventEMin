...
main(void)
{
  ...
  // model
  typedef model::SixDOF<T> Model;

  constexpr int ndims=Model::ndims(), nvars=Model::nvars();

  // you can modify the entropy used by uncommenting the corresponding line
  //typedef dispersion::Potential<Model> Dispersion;
  //typedef dispersion::Renyi<Model> Dispersion;
  //typedef dispersion::Shannon<Model> Dispersion;
  //typedef dispersion::SharmaMittal<Model> Dispersion;
  typedef dispersion::Tsallis<Model> Dispersion;

  // optimiser
  typedef optimiser::GSLfdfOptimiser<Dispersion> Optimiser;

  const std::string fdir("../dataset/indoor_flying1");
  ...
  event::unprojectEvents<T>(camParams, c);

  Dispersion dispersion;
  dispersion.assignPoints(c, ts, polarity);
  ...
  std::cout<<optimiser.run(vars)<<", w: "<<optimiser.vars().template head<3>().transpose()<<", v: "<<optimiser.vars().template tail<3>().transpose()<<'\n';

  const Model model;
  mtx<T> cm(ndims, nEvents);
  model(optimiser.vars(), c, ts, cm);
  const std::string fmodelled(fdir+"/events_modelled.txt");
  if(!event::save<T>(fmodelled, cm, ts, polarity))
  {
    std::cout<<"error writing events modelled to file "<<fmodelled<<'\n';
    return -1;
  }
